...
### 🎯 Classificação Binária de Lesões
O sistema agora utiliza o modelo **melaNet** para classificação binária:

- **Benigno**: Lesões benignas (não cancerosas)
- **Maligno**: Lesões malignas (cancerosas)

**Vantagens da Classificação Binária:**
- Maior precisão para detecção de câncer de pele
- Resultados mais claros e diretos
- Priorização automática (urgente para maligno, rotina para benigno)
- Baseado no modelo melaNet treinado especificamente para melanoma detection
...
### IA e Modelos
- **melaNet**: Modelo binário especializado em detecção de melanoma (hasibzunair/melanet)
...
### Versão 2.0 - Classificação Binária com melaNet
- [x] **Novo Modelo**: Substituição do classificador multiclasses pelo melaNet (hasibzunair/melanet)
- [x] **Classificação Binária**: Simplificação para Benigno/Maligno
- [x] **Priorização Automática**: Urgente para maligno, rotina para benigno
- [x] **Implementação Keras**: Suporte ao modelo TensorFlow/Keras
- [x] **Download Automático**: Baixamento automático do modelo do Hugging Face
- [x] **Fallback Simulado**: Modelo simulado quando o real não está disponível
- [x] **Novos Endpoints**: `/api/model-info` para informações do modelo

### Vantagens da Mudança:
- **Maior Precisão**: Modelo especializado em detecção de melanoma
- **Resultados Claros**: Classificação binária mais direta
- **Priorização Inteligente**: Urgência baseada no resultado
- **Compatibilidade**: Mantém interface da API existente
